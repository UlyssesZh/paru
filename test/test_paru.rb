...
require_relative '../lib/paru/error'
...
class ParuTest < Minitest::Test
  ...
  def run_converter(converter, input_file, output_file, use_output_option: false)
    ...
    converted_input = output if use_output_option
  ...
  def test_info
    ...
    if Gem.win_platform?
      ...
      if ((major >= 2) && (minor >= 7)) || (major >= 3)
        assert_match(%r{\.local/share/pandoc$}, info[:data_dir])
      else
        assert_match(/\.pandoc$/, info[:data_dir])
      end
    end
  ...
  def test_simple_conversion_with_spaces
    ...
    run_converter converter, 'test/pandoc_input/strong hi.md', 'test/pandoc_output/strong hi.html',
                  use_output_option: true
  end
  ...
  OPTION_PATTERN = /--[a-zA-Z-]+/m.freeze
