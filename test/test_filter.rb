...
require_relative '../lib/paru/filter/code_block'
require_relative '../lib/paru/filter/ordered_list'
require_relative '../lib/paru/filter/bullet_list'
require_relative '../lib/paru/filter/definition_list'
require_relative '../lib/paru/filter/table'
...
class FilterTest < Minitest::Test
  ...
  def test_inline_elements
    Dir.glob('test/pandoc_input/inline/*.md') do |path|
      ...
      output = filter_string(input) do
        with node_name.to_s do |node|
          if node_name.to_s == 'Image'
  ...
  def test_world_to_moon
    ...
    output = filter_string(input) do
      with 'Str' do |s|
        s.string = 'moon' if s.string == 'world'
      end
    end
  ...
  def test_table_convenience
    ...
    input_data = [%w[No String], %w[1 Hello], %w[2 Hi], %w[3 Goodbye]]
    input = 'replace this'
    ...
    output = filter_string(input) do
      with 'Para' do |p|
        table = Paru::PandocFilter::Table.from_file 'test/pandoc_output/table.csv', headers: true,
                                                                                    caption: 'A list of greetings.'
        p.parent.replace(p, table)
      end
    end
